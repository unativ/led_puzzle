...
byte shapes[][4][8]={
  ...
  // poo 12
  {
   {0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xf, 0x11},
   {0x0, 0x0, 0x0, 0x60, 0xc0, 0xc0, 0xe0, 0x10},
   {0x15, 0x15, 0x11, 0x1f, 0x10, 0x78, 0x7c, 0x7f},
   {0x50, 0x50, 0x10, 0xf8, 0x18, 0x3e, 0x7e, 0xfe}
  },
  // cat 13
  {
    {0x10, 0x28, 0x44, 0x44, 0x7f, 0x80, 0x88, 0x94},
    {0x8, 0x14, 0x22, 0x22, 0xfe, 0x1, 0x11, 0x29},
    {0x80, 0x41, 0x41, 0x25, 0x12, 0x10, 0xf, 0x0},
    {0x1, 0x82, 0x82, 0xa4, 0x48, 0x8, 0xf0, 0x0}
  },
  // smile 14
  {
    {0x0, 0x0, 0x7, 0x8, 0x10, 0x26, 0x26, 0x20},
    {0x0, 0x0, 0xe0, 0x10, 0x8, 0x64, 0x64, 0x4},
    {0x20, 0x20, 0x24, 0x13, 0x8, 0x7, 0x0, 0x0},
    {0x4, 0x4, 0x24, 0xc8, 0x10, 0xe0, 0x0, 0x0}
  },
  // frog 15
  {
    {0x3c, 0x42, 0x5a, 0x5a, 0xff, 0x80, 0x82, 0x80},
    {0x3c, 0x42, 0x5a, 0x5a, 0xff, 0x1, 0x41, 0x1},
    {0x5f, 0x50, 0x50, 0x4c, 0x33, 0x1c, 0x13, 0x10},
    {0xfa, 0xa, 0xa, 0x32, 0xcc, 0x38, 0xc8, 0x8}
  },
  // butterfly 16
  {
    {0x0, 0x0, 0x6, 0xa, 0x71, 0x89, 0xa5, 0x4b},
    {0x0, 0x0, 0x60, 0x50, 0x8e, 0x91, 0xa5, 0xd2},
    {0x31, 0xf, 0x5, 0xb, 0x15, 0x19, 0x0, 0x0},
    {0x80, 0xf0, 0xa0, 0xd0, 0xa8, 0x98, 0x0, 0x0}
  },
  // pokemon 17
  {
    {0x7, 0xf, 0x3f, 0x7f, 0x7f, 0xfe, 0xfd, 0xf9},
    {0xe0, 0xf0, 0xfc, 0xfe, 0xfe, 0x7f, 0xbf, 0x9f},
    {0x86, 0x82, 0x81, 0x40, 0x40, 0x30, 0x8, 0x7},
    {0x61, 0x41, 0x81, 0x2, 0x2, 0xc, 0x10, 0xe0}
  }
};
...
void newGame() {
  memcpy(UL, empty, 8 * sizeof(byte));
  memcpy(UR, empty, 8 * sizeof(byte));
  memcpy(DL, empty, 8 * sizeof(byte));
  memcpy(DR, empty, 8 * sizeof(byte));
  drawLeds();
  ...
  memcpy(UL, digits[3], 8 * sizeof(byte));
  drawLeds();
  delay(2000);
  digitalWrite(RED_PIN, LOW);
  digitalWrite(YELLOW_PIN, HIGH);
  memcpy(UL, empty, 8 * sizeof(byte));
  memcpy(UR, digits[2], 8 * sizeof(byte));
  drawLeds();
  delay(2000);
  digitalWrite(YELLOW_PIN, LOW);
  digitalWrite(GREEN_PIN, HIGH);
  memcpy(UR, empty, 8 * sizeof(byte));
  memcpy(DL, digits[1], 8 * sizeof(byte));
  drawLeds();
